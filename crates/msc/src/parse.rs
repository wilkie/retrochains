...
pub(crate) fn parse_function(p: &mut Parser<'_>) -> Result<Function, EmitError> {
    ...
    loop {
        ...
        loop {
            ...
            let spec = LocalSpec { size: slot_size, array_len: slot_len, init: None, struct_idx: None, is_long: is_long_slot, init_is_literal: false, is_far_ptr: is_far_ptr_decl, pointee_size: if star_count > 0 { size } else { 0 }, is_unsigned: has_unsigned && star_count == 0 && (size == 1 || is_long_slot), init_via_cast: false, init_via_type_cast: false, is_float: false, float_bits: None, block_offset: None };
            p.local_specs.push(spec.clone());
            locals.push(spec);
            if matches!(p.peek(), Some(Tok::Assign)) {
                p.bump();
