...
pub struct Unit {
    ...
    /// Per prototype-only declaration, the per-param struct-by-value byte sizes
...
pub struct Locals<'a> {
    ...
    /// Map of function symbol names to their per-param struct-by-value byte
    ...
    /// Set by `emit_loop` when the loop it just emitted is do-while-form with
    /// no break/continue: its body always runs, so AX at loop exit comes from
    /// the body's last AX write and a post-loop read may reuse it (fixture
...
pub enum AssignTarget {
    ...
    /// `<struct-local> = <struct-global>;` — whole-struct copy from a global
    /// into a stack local. `dst` is the local idx, `src` the global idx. ≤4
    /// bytes loads the global via AX/DX moffs and stores into the local;
    /// larger copies via `movsw` with `es=ss` (dst on stack). Fixtures 415/416.
    StructLocalFromGlobalCopy { dst: usize, src: usize, bytes: u16 },
    ...
    /// `*++<ptr-local> = <expr>;` — advance the pointer by `step` FIRST, then
    /// store through the NEW value. Codegen: `add [bp-p],step; mov bx,[bp-p];
    /// mov [bx],ax/imm`. Fixture 4285.
    DerefPreMutateLocal { local_idx: usize, step: i32 },
    /// `*++<ptr-param> = <expr>;` — advance the pointer parameter first, then
    /// store through the new value.
    DerefPreMutateParam { param_idx: usize, step: i32 },
...
pub fn build_obj(source_filename: &str, unit: &Unit) -> Vec<u8> {
    ...
    // Per-function struct-by-value param byte sizes (defined fns + prototypes),
